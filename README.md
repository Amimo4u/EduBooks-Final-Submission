EduBooks - Full-Stack E-commerce/Education Platform

This repository serves as the master entry point for the EduBooks project. The application is structured as two separate codebases deployed across Vercel (UI) and Render (API).

1. 🌐 Live Application Link

This is the URL to view the live, functional application and test the sign-up/login functionality.

Live UI URL (Frontend): https://edubooksfrontend.vercel.app/

2. 📁 Source Code Repositories

The project source code is separated into the following two dedicated repositories:

Component

Technology

Repository Link

Frontend (UI & Deployment Config)

React.js / Vercel

https://github.com/Amimo4u/edubooks-frontend

Backend (API & Database)

Node.js / Express / Render

https://github.com/Amimo4u/Edubooksbackend

3. 📝 Project Status and Details

The detailed breakdown of implemented features, successful deployments, and features is located in the Frontend Repository's README.md file.
